# -*- coding: utf-8 -*-
"""portfolio_management.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1BZ8vmw8tipUEP5p4nnvy4n-lwrvgUTxo
"""

from google.colab import drive
drive.mount('/content/drive')

"""Set Up SQLite Database"""

import os
os.remove("portfolio.db")  # Delete the existing database file

import sqlite3
from tabulate import tabulate

# Connect to an SQLite database
conn = sqlite3.connect("portfolio.db")
cursor = conn.cursor()

# Create table for financial assets
cursor.execute('''
CREATE TABLE IF NOT EXISTS financial_assets (
    asset_id INTEGER PRIMARY KEY AUTOINCREMENT,
    asset_name TEXT NOT NULL,
    asset_symbol TEXT UNIQUE NOT NULL,
    asset_type TEXT NOT NULL,
    purchase_price REAL NOT NULL,
    current_price REAL NOT NULL,
    purchase_date TEXT NOT NULL,
    quantity INTEGER NOT NULL
)
''')

conn.commit()
conn.close()

"""Install Yahoo Finance and Fetch Stock Prices"""

import sqlite3
from tabulate import tabulate  # Install via: pip install tabulate
import yfinance as yf  # Install via: pip install yfinance

def manage_assets():
    """Removes duplicate records, inserts sample data, and updates stock prices."""
    # Connect to SQLite database
    conn = sqlite3.connect("portfolio.db")
    cursor = conn.cursor()

    try:
        # Step 1: Remove duplicates, keeping the one with the lowest asset_id
        cursor.execute('''
        DELETE FROM financial_assets
        WHERE asset_id NOT IN (
            SELECT MIN(asset_id) FROM financial_assets GROUP BY asset_symbol
        )
        ''')


        # Step 2: Insert sample data
        assets = [
            ("Apple", "AAPL", "Stock", 174.49, 0, "2023-08-13", 10),
            ("Tesla", "TSLA", "Stock", 267.43, 0, "2023-07-31", 5),
            ("Amazon", "AMZN", "Stock", 140.39, 0, "2023-09-10", 8),
            ("NVIDIA", "NVDA", "Stock", 43.49, 0, "2023-09-01", 6),  # Added NVIDIA
            #("Microsoft", "MSFT", "Stock", 600.37, 0, "2023-09-10", 8),
        ]
        # Insert data only if asset_symbol does not already exist
        for asset in assets:
            cursor.execute("SELECT COUNT(*) FROM financial_assets WHERE asset_symbol = ?", (asset[1],))
            if cursor.fetchone()[0] == 0:  # If asset_symbol does not exist, insert it
                cursor.execute('''
                INSERT INTO financial_assets (asset_name, asset_symbol, asset_type, purchase_price, current_price, purchase_date, quantity)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', asset)
        print("Sample data inserted!")

        # Step 3: Update stock prices
        cursor.execute("SELECT asset_symbol FROM financial_assets")
        symbols = [row[0] for row in cursor.fetchall()]

        for symbol in symbols:
            try:
                stock = yf.Ticker(symbol)
                current_price = stock.history(period="1d")["Close"].iloc[-1]
                cursor.execute("UPDATE financial_assets SET current_price = ? WHERE asset_symbol = ?", (current_price, symbol))
                print(f"Stock price updated for {symbol}")
            except Exception as e:
                print(f"Error fetching data for {symbol}: {e}")

        # Commit the changes and close the connection
        conn.commit()

        # Step 4: Fetch and display only unique records
        cursor.execute('''
        SELECT asset_name, asset_symbol, asset_type, purchase_price, current_price, purchase_date, quantity
        FROM financial_assets
        GROUP BY asset_symbol
        ''')  # Ensures no duplicate asset symbols are shown

        rows = cursor.fetchall()
        conn.close()  # Close connection

        # Define table headers
        headers = ["Asset Name", "Symbol", "Type", "Purchase Price", "Current Price", "Purchase Date", "Quantity"]

        # Display results in a table format
        print("\nFinal portfolio data:\n")
        print(tabulate(rows, headers=headers, tablefmt="grid"))

    except Exception as e:
        print(f"An error occurred: {e}")
        if conn:
            conn.rollback()  # Rollback on error
    finally:
        if conn:
            conn.close()  # Ensure connection is closed

# Call the function
manage_assets()

"""Portfolio Performance Analysis"""

import sqlite3
import pandas as pd
from tabulate import tabulate  # Install via: pip install tabulate

def analyze_portfolio():
    conn = sqlite3.connect("portfolio.db")
    df = pd.read_sql_query("SELECT * FROM financial_assets", conn)
    conn.close()

    # Calculate total investment and current value
    df["total_investment"] = df["purchase_price"] * df["quantity"]
    df["current_value"] = df["current_price"] * df["quantity"]
    df["profit_loss"] = df["current_value"] - df["total_investment"]
    df["return_%"] = (df["profit_loss"] / df["total_investment"]) * 100

    # Portfolio summary
    total_investment = df["total_investment"].sum()
    total_value = df["current_value"].sum()
    overall_return = ((total_value - total_investment) / total_investment) * 100

    # Print portfolio details in table format
    headers = ["Asset Name", "Total Investment", "Current Value", "Profit/Loss", "Return (%)"]
    portfolio_table = df[["asset_name", "total_investment", "current_value", "profit_loss", "return_%"]]

    print("\nPortfolio Analysis:\n")
    print(tabulate(portfolio_table, headers=headers, tablefmt="grid"))

    # Print portfolio summary
    print(f"\nTotal Investment: ${total_investment:,.2f}")
    print(f"Current Portfolio Value: ${total_value:,.2f}")
    print(f"Overall Portfolio Return: {overall_return:.2f}%")

# Call the function
analyze_portfolio()

"""Visualize Portfolio Performance"""

import matplotlib.pyplot as plt

def visualize_portfolio():
    conn = sqlite3.connect("portfolio.db")
    df = pd.read_sql_query("SELECT asset_name, purchase_price, current_price FROM financial_assets", conn)
    conn.close()

    df["profit_loss"] = df["current_price"] - df["purchase_price"]

    plt.figure(figsize=(8, 5))
    plt.bar(df["asset_name"], df["profit_loss"], color=["green" if x > 0 else "red" for x in df["profit_loss"]])
    plt.xlabel("Assets")
    plt.ylabel("Profit/Loss ($)")
    plt.title("Portfolio Performance")
    plt.show()

visualize_portfolio()

"""Automate Risk Analysis"""

import numpy as np

def calculate_risk():
    conn = sqlite3.connect("portfolio.db")
    df = pd.read_sql_query("SELECT asset_name, purchase_price, current_price FROM financial_assets", conn)
    conn.close()

    df["daily_return"] = (df["current_price"] - df["purchase_price"]) / df["purchase_price"]
    portfolio_risk = np.std(df["daily_return"]) * 100  # Convert to %

    print(f"Portfolio Volatility (Risk): {portfolio_risk:.2f}%")

calculate_risk()

"""Predict Stock Prices using LSTM (Long Short-Term Memory) Networks."""

!pip install yfinance tensorflow scikit-learn

"""Fetch and Prepare Stock Data for LSTM"""

import yfinance as yf
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from sklearn.preprocessing import MinMaxScaler
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense

# Fetch historical stock data
def get_stock_data(symbol, period="1y"):
    stock = yf.Ticker(symbol)
    df = stock.history(period=period)
    df = df[['Close']]  # We only need closing prices
    return df

# Example: Fetch Apple stock data
df = get_stock_data("AAPL")
print(df.head())

# Normalize data for LSTM
scaler = MinMaxScaler(feature_range=(0,1))
df_scaled = scaler.fit_transform(df)

# Prepare dataset for LSTM
def create_sequences(data, seq_length=50):
    X, y = [], []
    for i in range(len(data) - seq_length):
        X.append(data[i:i+seq_length])
        y.append(data[i+seq_length])
    return np.array(X), np.array(y)

seq_length = 50
X, y = create_sequences(df_scaled, seq_length)

# Split into training and testing sets
train_size = int(len(X) * 0.8)
X_train, y_train = X[:train_size], y[:train_size]
X_test, y_test = X[train_size:], y[train_size:]

# Reshape for LSTM input
X_train = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
X_test = X_test.reshape((X_test.shape[0], X_test.shape[1], 1))

"""Build & Train the LSTM Model"""

# Define LSTM model
model = Sequential([
    LSTM(units=50, return_sequences=True, input_shape=(seq_length, 1)),
    LSTM(units=50, return_sequences=False),
    Dense(units=25),
    Dense(units=1)
])

model.compile(optimizer='adam', loss='mean_squared_error')

# Train the model
model.fit(X_train, y_train, epochs=20, batch_size=16, validation_data=(X_test, y_test))

"""Predict Future Prices"""

# Make predictions
predictions = model.predict(X_test)

# Transform predictions back to original scale
predictions = scaler.inverse_transform(predictions)
y_test_actual = scaler.inverse_transform(y_test.reshape(-1, 1))

# Plot results
plt.figure(figsize=(10, 5))
plt.plot(y_test_actual, label="Actual Prices")
plt.plot(predictions, label="Predicted Prices")
plt.legend()
plt.title("Stock Price Prediction using LSTM")
plt.show()

"""Optimize Portfolio using Modern Portfolio Theory (MPT)

MPT helps allocate investments to maximize return while minimizing risk.
"""

!pip install numpy pandas matplotlib scipy

""" Portfolio Optimization Code"""

import numpy as np
import pandas as pd
from scipy.optimize import minimize
from tabulate import tabulate

# Example: 3 assets (Apple, Tesla, Amazon)
assets = ["AAPL", "TSLA", "AMZN"]

# Assuming get_stock_data is a function that fetches stock data (replace it with actual data fetching)
def get_stock_data(symbol, period="1y"):
    return yf.download(symbol, period=period)["Close"]

# Fetch stock data and compute returns
df = pd.concat([get_stock_data(asset, period="1y") for asset in assets], axis=1)
df.columns = assets

# Compute daily returns
returns = df.pct_change().dropna()

# Mean & covariance matrix
mean_returns = returns.mean()
cov_matrix = returns.cov()

# Define portfolio risk & return
def portfolio_performance(weights, mean_returns, cov_matrix):
    returns = np.sum(mean_returns * weights) * 252  # Annualized return
    risk = np.sqrt(np.dot(weights.T, np.dot(cov_matrix * 252, weights)))  # Annualized volatility
    return returns, risk

# Optimization: Maximize Sharpe Ratio
def neg_sharpe_ratio(weights, mean_returns, cov_matrix, risk_free_rate=0.01):
    returns, risk = portfolio_performance(weights, mean_returns, cov_matrix)
    return -((returns - risk_free_rate) / risk)

# Constraints & bounds
num_assets = len(assets)
constraints = {"type": "eq", "fun": lambda x: np.sum(x) - 1}
bounds = tuple((0, 1) for _ in range(num_assets))
initial_weights = np.array([1 / num_assets] * num_assets)

# Optimize
opt_results = minimize(neg_sharpe_ratio, initial_weights, args=(mean_returns, cov_matrix),
                       method="SLSQP", bounds=bounds, constraints=constraints)

# Optimized weights
optimized_weights = opt_results.x
optimized_returns, optimized_risk = portfolio_performance(optimized_weights, mean_returns, cov_matrix)

# Create the table
allocation_data = [(asset, f"{weight:.2%}") for asset, weight in zip(assets, optimized_weights)]
headers = ["Asset", "Optimized Weight"]

# Print results as a table
print("\nOptimized Portfolio Allocation:\n")
print(tabulate(allocation_data, headers=headers, tablefmt="grid"))

# Portfolio performance summary
summary_data = [
    ("Expected Return", f"{optimized_returns:.2%}"),
    ("Expected Risk", f"{optimized_risk:.2%}")
]

# Print performance summary in table format
summary_headers = ["Metric", "Value"]
print("\nPortfolio Performance Summary:\n")
print(tabulate(summary_data, headers=summary_headers, tablefmt="grid"))

"""Automate Alerts for Asset Performance"""

import smtplib

def send_email_alert(asset, current_price, threshold):
    sender_email = "your_email@gmail.com"
    receiver_email = "your_email@gmail.com"
    password = "your_app_password"  # Generate an app password for Gmail SMTP

    subject = f"ALERT: {asset} Price Drop"
    body = f"The price of {asset} has fallen to ${current_price}, below the threshold of ${threshold}."

    email_message = f"Subject: {subject}\n\n{body}"

    try:
        server = smtplib.SMTP("smtp.gmail.com", 587)
        server.starttls()
        server.login(sender_email, password)
        server.sendmail(sender_email, receiver_email, email_message)
        server.quit()
        print(f"Alert sent for {asset}!")
    except Exception as e:
        print(f"Failed to send email: {e}")

# Example: Monitor stock price
def monitor_stocks():
    conn = sqlite3.connect("portfolio.db")
    cursor = conn.cursor()
    cursor.execute("SELECT asset_name, asset_symbol, current_price FROM financial_assets")
    stocks = cursor.fetchall()
    conn.close()

    price_thresholds = {"AAPL": 130, "TSLA": 180, "AMZN": 100}

    for stock in stocks:
        asset, symbol, price = stock
        if symbol in price_thresholds and price < price_thresholds[symbol]:
            send_email_alert(asset, price, price_thresholds[symbol])

monitor_stocks()